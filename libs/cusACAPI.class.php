...
class cUsACComAPI {
    ...
    public function createCustomer($postData){
        ...
        elseif  ( !strlen($postData['website']) ):    echo ' Missing Website, a is required field! ';
            ...
            $strCURLOPT .= '&ActiveCampaign_Delivery_Api_Url='.trim($postData['AC_apiurl']);
            $strCURLOPT .= '&ActiveCampaign_Delivery_Api_Key='.trim($postData['AC_apikey']);
            $strCURLOPT .= '&ActiveCampaign_Delivery_Unique_List_ID='.trim($postData['listID']);
            $strCURLOPT .= '&IP_Address='.$this->getIP();
            $strCURLOPT .= '&Auto_Activate=1';
            $strCURLOPT .= '&Template_Desktop_Form=newsletterTemplate1';
            $strCURLOPT .= '&Promotion_Code=WPAC';
            $strCURLOPT .= '&Version=ac|1.0.3';
    ...
    public function updateDeliveryOptions($cUs_email, $cUs_pass, $postData, $formkey){
        ...
        $strCURLOPT .= '&ActiveCampaign_Delivery_Enabled=1';
        $strCURLOPT .= '&ActiveCampaign_Delivery_Api_Url='.trim($postData['AC_apiurl']);
        $strCURLOPT .= '&ActiveCampaign_Delivery_Api_Key='.trim($postData['AC_apikey']);
        $strCURLOPT .= '&ActiveCampaign_Delivery_Unique_List_ID='.trim($postData['listID']);
    ...
    public function resetData(){
        delete_option( 'cUsAC_settings' );
        delete_option( 'cUsAC_FORM_settings' );
        delete_option( 'cUsAC_settings_api_key' );
        delete_option( 'cUsAC_settings_step1' ); 
        delete_option( 'cUsAC_settings_form_key' );
